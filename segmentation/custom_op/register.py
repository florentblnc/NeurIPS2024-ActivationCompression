...
def add_svd_filter(module: nn.Module, cfg):
    if 'svd_size' in cfg:
        if cfg['type'] == 'cbr':
            module.conv = wrap_convSVD(module.conv, cfg['SVD_var'], True, cfg["svd_size"])
        elif cfg['type'] == 'resnet_basic_block':
            module.conv1 = wrap_convSVD(module.conv1, cfg['SVD_var'], True, cfg["svd_size"])
            module.conv2 = wrap_convSVD(module.conv2, cfg['SVD_var'], True, cfg["svd_size"])
        elif cfg['type'] == 'conv':
            module = wrap_convSVD(module, cfg['SVD_var'], True, cfg["svd_size"])
        else:
            raise NotImplementedError
    else:
        if cfg['type'] == 'cbr':
            module.conv = wrap_convSVD(module.conv, cfg['SVD_var'], True)
        elif cfg['type'] == 'resnet_basic_block':
            module.conv1 = wrap_convSVD(module.conv1, cfg['SVD_var'], True)
            module.conv2 = wrap_convSVD(module.conv2, cfg['SVD_var'], True)
        elif cfg['type'] == 'conv':
            module = wrap_convSVD(module, cfg['SVD_var'], True)
        else:
            raise NotImplementedError
    return module
