...
logger_args="--logger.save_dir runs/setup$setup/swinT/$dataset/HOSVD/var$var"
...
echo $common_args
# There are 36 convolutional layers in swinT
python trainer_cls_linear.py ${common_args} --logger.exp_name HOSVD_l2_var${var}_${usr_group_kl} --model.num_of_finetune 2
python trainer_cls_linear.py ${common_args} --logger.exp_name HOSVD_l4_var${var}_${usr_group_kl} --model.num_of_finetune 4