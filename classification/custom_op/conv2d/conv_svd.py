...
def restore_tensor(Uk_Sk, Vk_t, shape):
    """
    Restore the original tensor from SVD decomposed matrices.

    Args:
        Uk_Sk (Tensor): Left singular vectors multiplied by singular values.
        Vk_t (Tensor): Transposed right singular vectors.
        shape (Tuple): The shape of the original tensor.
    
    Returns:
        Tensor: The reconstructed tensor with the original shape.
    """
...
class Conv2d_SVD_op(Function):
    """
    Custom function that applies convolution and Singular Value Decomposition (SVD)
    on the input tensor during the forward pass, and reconstructs the tensor during
    the backward pass for gradient calculations.
    """
...
def wrap_convSVD(conv, SVD_var, active, svd_size):
    """
    ...
    Args:
        ...
        svd_size (list): A list to log the SVD size during training.
